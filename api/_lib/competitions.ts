...
import type { Competition, CompetitionListItem } from '../../shared/schemas.js'
...
/** Todas las competiciones con su número de partidos jugados. */
export async function listCompetitions(): Promise<CompetitionListItem[]> {
  const { data, error } = await db()
    .from('competitions')
    .select('id,name,kind,matches(count)')
    .eq('org_id', env.orgId)
  if (error) throw error
  return data
    .map((row) => ({ ...toCompetition(row), match_count: row.matches[0]?.count ?? 0 }))
