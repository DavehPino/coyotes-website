...
export function TeamStatBars({ us, them, usLabel, themLabel }: TeamStatBarsProps) {
  if (!us && !them) {
    return <p className="text-sm text-ink-soft">CourtTrack no registró estadísticas de este set.</p>
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between gap-4 text-xs font-semibold text-ink">
      ...
      <ul className="flex flex-col gap-2">
        {STAT_COLUMNS.map(({ key, label }) => {
          ...
          return (
            ...
            >
              <span className="text-right font-semibold tabular-nums text-ink">{left}</span>
              <span className="h-2.5">
                <span
                  className="block h-full bg-ink transition-[clip-path] duration-300 ease-out"
              ...
              <span className="h-2.5">
                <span
                  className="block h-full bg-ink-soft transition-[clip-path] duration-300 ease-out"
                  style={{ clipPath: `inset(0 ${100 - percent(right)}% 0 0 round 0 4px 4px 0)` }}
                />
              </span>
              <span className="font-semibold tabular-nums text-ink">{right}</span>
            </li>
          )
        })}
      </ul>

      <dl className="grid grid-cols-2 gap-x-4 gap-y-1 border-t border-ink/15 pt-3 text-xs text-ink-soft">
        {(['us', 'them'] as const).map((side) => {
          ...
          return (
            <div key={side} className={side === 'them' ? 'text-right' : ''}>
              ...
              <dd>
                <span className="font-bold text-2xl leading-none text-ink tabular-nums">{line?.points ?? '–'}</span>{' '}
                puntos
              </dd>
              {parts && (
