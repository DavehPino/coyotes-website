...
import { CalendarIcon, MapPinIcon, PlusIcon } from '../ui/icons'
...
/** Zona de ataque: lo siguiente en grande, y lo que viene después como filas separadas por cinta. */
    ...
    <Zone id="home-next-up" label="Próximo partido" more={{ to: '/activities', label: 'Agenda' }}>
      {query.isPending ? (
        ...
        <div className="flex flex-col">
          ...
          {following.length > 0 && (
            <ul className="divide-tape tape-rule mt-5">
...
function Featured({ activity, today, onOpen }: { activity: Activity; today: string; onOpen: (activity: Activity) => void }) {
  ...
  return (
    <article className="relative flex flex-col gap-3">
      ...
      {/* La fecha y la hora a escala de número de cancha, en stencil. */}
      <p className="flex flex-wrap items-baseline gap-x-4 font-stencil leading-none text-ink uppercase">
...
function FollowingRow({ activity, today, onOpen }: { activity: Activity; today: string; onOpen: (activity: Activity) => void }) {
  ...
  return (
    <button
      ...
      className={[
        'grid min-h-14 w-full grid-cols-[3.5rem_minmax(0,1fr)_auto] items-center gap-3 px-1 py-2 text-left',
        'transition-colors duration-150 ease-out hover:bg-line/50',
      ].join(' ')}
    >
      <span aria-hidden className="flex flex-col leading-tight uppercase">
        <span className="text-lg font-extrabold text-ink">{formatDayMonth(activity.activity_date)}</span>
...
function NextMatchSkeleton() {
  return (
    <div className="flex flex-col gap-3" aria-busy aria-label="Cargando próximas actividades">
      ...
      <div className="divide-tape tape-rule mt-4">
        {[0, 1, 2].map((i) => (
